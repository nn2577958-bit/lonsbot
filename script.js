...
<head>
  ...
  <style>
    ...
    #auth-section button:hover { background: #555; }

    .error { color: red; text-align: center; margin-bottom: 10px; }

    #user-info { display: none; margin-bottom: 10px; padding: 10px; background: #f1f1f1; border-radius: 5px; text-align: center; }
...
<body>
  ...
  <main>
    <section id="auth-section">
      ...
      <h2>Google 로그인</h2>
      <button id="google-login">Google 계정으로 로그인</button>
      <p id="google-msg" class="error"></p>

      <button id="logout-btn">로그아웃</button>
      <button id="delete-btn">계정 삭제</button>
      <p id="status-msg" style="text-align:center;"></p>
    </section>
  ...
  <script type="module">
    ...
    signupForm.addEventListener("submit", e => {
      ...
      createUserWithEmailAndPassword(auth, email, pw)
        .then(() => { signupMsg.innerText="회원가입 완료! 로그인 해주세요."; signupForm.reset(); signupMsg.className=""; })
        .catch(err => {
    ...
    loginForm.addEventListener("submit", e => {
      ...
      signInWithEmailAndPassword(auth, email, pw)
        .then(() => { loginMsg.innerText="로그인 성공!"; loginForm.reset(); loginMsg.className=""; })
    ...
    deleteBtn.addEventListener("click", () => {
      ...
      if(user){
        deleteUser(user).then(()=> alert("계정 삭제 완료!"))
